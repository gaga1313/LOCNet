...
class hConvGruCell(nn.Module):
    def __init__(
        ...
        self.w_gate_inh.register_hook(
            lambda grad: (grad + torch.transpose(grad, 1, 0)) * 0.5
        )
        self.w_gate_exc.register_hook(
            lambda grad: (grad + torch.transpose(grad, 1, 0)) * 0.5
        )
        ...
        if self.batchnorm:
            ...
            self.bn = nn.ModuleList(
                [nn.BatchNorm2d(self.hidden_size, eps=1e-03) for i in range(32)]
            )
        else:
    ...
    def forward(self, input_, prev_state2, timestep=0):
        ...
        if self.batchnorm:
            ...
            c2_t = self.bn[i * 4 + 3](
                F.conv2d(next_state1, self.w_gate_exc, padding=self.padding)
            )
            h2_t = F.relu(
                self.kappa * next_state1
                + self.gamma * c2_t
                + self.w * next_state1 * c2_t
            )
            ...
            h2_t = F.tanh(
                self.kappa * (next_state1 + self.gamma * c2_t)
                + (self.w * (next_state1 * (self.gamma * c2_t)))
            )
...
class hConvGru(nn.Module):
    def __init__(self, timesteps=8, filt_size=9):
        ...
        self.conv0 = nn.Conv2d(
            3, self.hidden_size, kernel_size=7, padding=3
        )  ## previous gpu implementation had serre gabor initialization

        self.bnip = nn.BatchNorm2d(self.hidden_size)
        self.rnncell1 = hConvGruCell(
            self.hidden_size, self.hidden_size, kernel_size=self.kernel_size
        )
        self.bnop = nn.BatchNorm2d(self.hidden_size)

        self.conv6 = nn.Conv2d(
            self.hidden_size, 2, kernel_size=1
        )  ## previous gpu implementation had xavier intialization
...
class hConvGruResNet(nn.Module):
    def __init__(self):
        ...
        self.conv0_1 = nn.Conv2d(
            3, 32, kernel_size=3, stride=2, padding=1, dtype=self.dtype
        )
        self.conv0_2 = nn.Conv2d(
            32, self.hidden_size, kernel_size=3, stride=2, padding=1, dtype=self.dtype
        )
        ...
        self.bninp_2 = nn.BatchNorm2d(
            self.hidden_size, momentum=0.9, eps=1e-5, dtype=self.dtype
        )
        self.bninp_3 = nn.BatchNorm2d(
            self.hidden_size, momentum=0.9, eps=1e-5, dtype=self.dtype
        )
        ...
        self.bn1 = nn.BatchNorm2d(
            self.hidden_size * 2, momentum=0.9, eps=1e-5, dtype=self.dtype
        )
        self.rnncell2 = hConvGruCell(
            self.hidden_size * 2, self.hidden_size * 2, kernel_size=self.kernel_size
        )
        ...
        self.bn2 = nn.BatchNorm2d(
            self.hidden_size * 4, momentum=0.9, eps=1e-5, dtype=self.dtype
        )
        self.rnncell3 = hConvGruCell(
            self.hidden_size * 4, self.hidden_size * 4, kernel_size=self.kernel_size
        )
        ...
        self.bn3 = nn.BatchNorm2d(
            self.hidden_size * 8, momentum=0.9, eps=1e-5, dtype=self.dtype
        )
        self.rnncell4 = hConvGruCell(
            self.hidden_size * 8, self.hidden_size * 8, kernel_size=self.kernel_size
        )
        ...
        self.bnop_1 = nn.BatchNorm2d(
            self.hidden_size * 16 * 2, momentum=0.9, eps=1e-5, dtype=self.dtype
        )
        # self.bnop_2 = nn.BatchNorm(momentum=0.9, epsilon=1e-5, dtype=self.dtype)
        self.bnop_3 = nn.BatchNorm1d(
            self.hidden_size * 16 * 2, momentum=0.9, eps=1e-5, dtype=self.dtype
        )
